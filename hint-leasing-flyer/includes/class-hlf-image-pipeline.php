...
final class HLF_Image_Pipeline {
	...
	/**
	 * 업로드 원본의 긴 변 상한(코어 기본값 2560보다 낮게 잡아 저장 용량/처리 비용을 줄인다).
	 *
	 * 예전 값 900px은 실제로 버그의 원인이었다(외부 코드 감사 P1) — 이 플러그인이 등록한 갤러리
	 * 대표 사이즈 hlf-item-photo가 960×640 하드크롭인데(class-hlf-plugin.php), 원본을 900px로 먼저
	 * 줄여버리면 워드프레스는 이미지를 확대하지 않으므로 960px짜리 크롭을 아예 만들 수 없다. 그러면
	 * 코어가 요청한 사이즈 대신 다른 등록 사이즈들로 srcset을 채우게 되고, 그 srcset이 남아 있으면
	 * 브라우저가 src 교체를 무시해 "갤러리 호버해도 사진이 안 바뀐다"는 실사용 버그로 이어졌다
	 * (v0.4.0-beta.12에서 srcset을 지우는 쪽으로 증상을 막았지만, 근본 원인은 이 상한이었다).
	 *
	 * 1920px이면 960×640 크롭을 항상 여유 있게 만들 수 있고(고해상도 화면의 2배 밀도까지 커버),
	 * 그래도 스마트폰 원본(4032px 등)보다는 훨씬 작아 용량 이득은 그대로 남는다.
	 */
	const MAX_ORIGINAL_DIMENSION = 1920;
