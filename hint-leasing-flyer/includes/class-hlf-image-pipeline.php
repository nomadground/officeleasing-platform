...
final class HLF_Image_Pipeline {
	...
	// 상한(8000px)은 "적당히 최적화" 요청에 맞춰 정확한 값을 지정하진 않았지만, 실제 최적화는 위
	// 900px 자동 리사이즈가 담당하므로 여기서는 압축 폭탄(가로/세로가 비정상적으로 큰 이미지가 서버
	// 리사이즈 처리 중 메모리를 과도하게 잡아먹는 것)만 막는 안전망 성격으로 넉넉하게 둔다.
	const MAX_UPLOAD_BYTES     = 1 * 1024 * 1024; // 1MB
	const MAX_UPLOAD_DIMENSION = 8000; // px(가로/세로 각각의 상한).
