...
( function () {
	...
	var PRINT_MAP_SIZES = {
		...
		// 잡았는데, 실사용 확인 결과 사진·지도가 너무 좁아 2배(42mm)로 키운 뒤(beta.26), 좁다는 요청이
		// 다시 들어와 42mm -> 84mm로 한 번 더 2배 키웠는데, 실제 기기에서는 그만큼 커진 지도 때문에
		// 원래 1페이지에 2건씩 나오던 게 2페이지로 밀렸다는 요청 — 지도 높이만(사진 높이는 그대로)
		// 현재값의 70% 수준인 59mm로 낮춘다(84 x 0.7 = 58.8 반올림). 이 저장소의 Playwright 실측
		// 환경은 실제 폰트(Pretendard)보다 성긴 대체 폰트를 쓰므로 여기서 나오는 절대 높이값은
		// 실제 기기와 다르다(README 여러 차례 기록) — "70%로 줄인다"는 요청 자체를 그대로 반영했다.
		detail: { width: '178mm', height: '59mm' },
	};
	var printSizedMaps = [];
