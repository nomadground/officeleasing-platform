...
( function () {
	...
	//
	// 요청서(실사용 버그, beta.20 이후에도 재현): 방향이 세로로 강제된 상태에서는 매물 상세 페이지의
	// 지도가 "가운데 정사각형 정도만 보이고 양옆은 안 보이는" 형태로 잘렸다 — 지도 자체의 크기(위
	// mm 크기, position:relative)는 정확히 못박아 뒀는데도 이 증상이 남은 이유는 지도 폭이 아니라
	// 그 지도를 담은 상세 히어로 레이아웃(.hlf-detail-hero, 사진|지도 2단 grid) 자체가 가로(281mm)
	// 폭 전제로 짜여 있었기 때문이다 — 우리가 요청한 가로 방향을 브라우저/인쇄 파이프라인이 실제로는
	// 무시하고 세로 용지에 그대로 인쇄해 버리면(이건 CSS로 강제할 수 없는 OS/브라우저 인쇄 파이프라인
	// 자체의 알려진 한계다 — @page 힌트를 아예 무시하는 경우가 실사용에서 확인됨), 281mm 폭으로 그려진
	// 페이지 전체가 194mm 세로 용지에 억지로 맞춰지면서 좌우 바깥쪽(사진의 왼쪽 끝·지도의 오른쪽 끝
	// 등)이 잘려나가고 중앙 부분만 남는다. 폭을 아무리 안전하게 잡아도 "2단으로 나란히"라는 레이아웃
	// 자체가 세로 용지에서 살아남지 못하므로, 모바일 인쇄에서는 아예 사진 위/지도 아래로 세로로
	// 쌓는다(print.css body.hlf-print-map-sizing .hlf-detail-hero) — 그러면 필요한 최대 폭이 항상
	// 194mm 세로 용지 폭 이내로 들어와 어느 쪽으로 인쇄되든 잘릴 일이 없다. 그 대신 지도 폭도 2단의
	// 절반(85mm)이 아니라 비교 지도와 같은 전체 폭(178mm)으로 넓힌다.
	var PRINT_MAP_SIZES = {
		// 목록 페이지 2페이지의 "위치 확인" 비교 지도(패널 하나가 페이지 폭을 다 쓴다).
		comparison: { width: '178mm', height: '78mm' },
		// 매물 상세의 개별 지도 — 모바일 인쇄에서는 사진 아래로 세로로 쌓이므로 비교 지도와 같은
		// 전체 폭을 쓴다. 높이는 세로 쌓기로 늘어난 페이지 예산을 최대한 되찾도록 비교 지도(78mm,
		// 매물 여러 개를 한 화면에 담아야 함)보다 훨씬 낮게 잡는다 — 여기는 마커 하나만 보여주면
		// 되므로 78mm처럼 넓을 필요가 없다. Playwright 실측(가짜 매물 1건, 사진 있음): 58mm는 갤러리
		// 76mm와 합쳐 예산을 넘어 2페이지로 밀려났고, 45mm는 1페이지 안에 들어왔다.
		detail: { width: '178mm', height: '45mm' },
	};
	var printSizedMaps = [];
