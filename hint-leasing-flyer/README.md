...
## 요청서 반영 — v0.4.0-beta.21

### 모바일 인쇄 매물 상세 지도가 "가운데 정사각형 정도만 보이고 양옆은 안 보이는" 문제
beta.20까지의 수정(지도 컨테이너 position:relative + overflow:hidden)은 지도가 다른 페이지 위로
"넘쳐 나오는" 문제는 해결했지만, 지도 자체가 상자 안에서 잘려 보이는 이 증상은 남아 있었다.

**원인 재정리**: 매물 상세 히어로(`.hlf-detail-hero`, 사진|지도 2단 grid)는 A4 가로(281mm 사용
폭)를 전제로 짜여 있다. 그런데 `@page`에 `size: A4 landscape`를 명시해도 이를 무시하고 세로
용지로 인쇄하는 안드로이드/브라우저 인쇄 파이프라인이 실사용에서 확인됐다(아래 항목 참고, CSS로
더 강제할 수단이 없는 OS/브라우저 경계). 이 경우 브라우저는 여전히 281mm 폭으로 페이지를 그리고,
그 결과물이 194mm 세로 용지에 맞춰지는 과정에서 페이지 좌우 바깥쪽(2단 중 왼쪽 칸의 왼쪽 끝,
오른쪽 칸의 오른쪽 끝)이 잘려나가고 중앙 부분만 남는다 — 반면 목록 페이지의 비교 지도는 애초에
페이지 전체 폭 대비 훨씬 좁고(178mm) 항상 가운데 정렬(margin:auto)이라 이 잘림에서 비켜나 있었다
(리스트/차트 페이지는 이 증상이 보고되지 않은 이유).

**수정**: 모바일(≤700px, 인쇄 버튼을 눌렀을 때 JS가 판정)에서는 매물 상세의 2단 히어로를 포기하고
사진 위 · 지도 아래로 세로로 쌓는다(`body.hlf-print-map-sizing .hlf-detail-hero`,
`grid-template-columns: 1fr`) — 이러면 필요한 최대 폭이 항상 194mm 세로 용지 폭 이내로 들어와
어느 방향으로 실제 인쇄되든 좌우가 잘릴 일이 없다. 지도 폭도 2단의 절반(85mm)이 아니라 전체 폭
(178mm, 비교 지도와 동일)으로 넓히고, 늘어난 세로 공간 예산을 되찾기 위해 지도 높이는 45mm로
낮췄다(마커 하나만 보여주면 되므로 여러 매물을 담아야 하는 비교 지도의 78mm처럼 넓을 필요가 없다).
데스크톱(이 클래스가 전혀 붙지 않음)은 지금까지와 완전히 동일하게 2단 그대로 유지된다.

Playwright 실측: A4 가로/세로 두 content 폭(1062px/733px) 모두에서 모바일 모드는 항상 단일 열
(`grid-template-columns` 1030px/701px 하나)로 렌더링되고 가로 오버플로 0, 지도 박스는 673x170px
(178x45mm)로 고정된다. 페이지 수는(사진 1장 + Leasing Info + Property Details 4항목 기준 합성
테스트) 데스크톱/모바일 모두 1페이지로 동일하다 — 다만 실제 콘텐츠(매물특징 텍스트, 항목 수 등)가
더 많으면 모바일 쪽이 2페이지로 밀려날 여지는 남아 있다(지도가 잘려 보이는 것보다는 나은 절충).

### 모바일 인쇄 용지 방향이 여전히 세로로 고정되는 문제 — 알려진 한계
`@page { size: A4 landscape }`로 옮긴 뒤에도(beta.19) 특정 기기/인쇄 서비스에서 여전히 세로로
인쇄된다는 보고를 받았다. 조사 결과 이는 Chromium/Android 인쇄 파이프라인의 오래된 공개 이슈로,
페이지가 CSS로 가로를 요청해도 인쇄 대화상자가 세로를 먼저 선택해 두면 그 값을 그대로 쓰고 CSS
힌트를 무시하는 사례가 다수 보고돼 있다(Chromium 이슈 79847 등). **이건 웹페이지의 CSS/JS로 강제할
수 있는 범위 밖 — 브라우저/OS 인쇄 스택의 결정이라 우리 쪽에서 더 밀어붙일 CSS 레버가 없다.**
실무 대응: 인쇄 대화상자를 펼치면(⌄ 화살표) 보통 "방향" 드롭다운이 있다 — 있다면 거기서 가로를
직접 선택한다. 그 옵션 자체가 없는 기기/인쇄 서비스라면 "PDF로 저장" 후 별도 PDF 뷰어 앱에서
그 PDF를 열어 인쇄하는 경로가 종종 더 안정적이다(PDF 자체에는 이미 올바른 가로 페이지 크기가
박혀 있으므로). 위 지도 잘림 수정은 이 방향 문제가 실제로 발생하더라도(즉 최악의 경우를 가정해도)
내용이 잘리지 않도록 만든 것이다.
