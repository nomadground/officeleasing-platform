...
주소 칸이 그만큼 좁아지면서 생기는 두 가지 부작용은 폭이 아니라 글자 크기로 해결했다(폭을 다시
흔들면 두 줄의 칸 경계가 어긋나므로): 360px 안팎에서 지번주소가 두 줄로 넘어가던 것은 380px
이하에서만 1px 줄여서, 도로명주소 끝이 말줄임되던 것은 모바일에서 11px→10px로 줄여서 되돌렸다.
Playwright 실측(360/390/412px)으로 칸 경계 오차 0px, 주소 한 줄 유지, 값 잘림 없음을 확인했다.
