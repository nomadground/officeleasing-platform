...
## 요청서 반영 — v0.4.0-beta.23 (모바일 인쇄 전면 재설계: 세로 용지 기준)

이전까지는 모바일 인쇄가 실제로 세로로 나오는데도 우리 CSS(`@page`)는 계속 가로를 요청해, 브라우저가
실제로 할 일과 우리가 계산한 페이지 크기가 어긋나 있었다 — 지도가 가운데만 보이거나(2단 히어로가
281mm 가로 폭 전제), 여백이 충분한데도 푸터가 다음 페이지로 밀리는 증상(194mm 세로 높이만 반영된
페이지 나눔 계산) 모두 이 불일치가 원인이었다. 이번에는 반대로 접근한다 — **모바일은 세로로
나온다는 현실을 받아들이고, 우리도 세로를 요청해 브라우저의 계산과 실제 출력을 일치시킨다.**

### 1. 모바일 인쇄는 이제 우리도 세로(A4 portrait)를 명시한다
데스크톱은 지금까지처럼 가로(`@page`, assets/css/public.css)를 그대로 쓴다 — 잘 나온다는 확인을
받았으므로 손대지 않았다. 모바일(인쇄 버튼을 눌렀을 때 화면 폭이 700px 이하였던 경우, 아직 화면
상태라 실제 기기 폭을 정확히 아는 시점)에서만 JS가 `<style>@page{size:A4 portrait}</style>`를
`<head>`에 추가해 기존 가로 선언을 덮어쓴다(`applyMobilePagePortrait`, 인쇄가 끝나면 제거). 이제
브라우저가 실제로 하게 될 일(세로 인쇄)과 우리가 계산하는 페이지 크기(194mm 폭 x 281mm 높이)가
일치하므로, 아래 레이아웃들이 실제 출력과 정확히 맞아떨어진다.

### 2. 매물 상세 히어로(사진|지도)를 세로로 쌓는다
194mm 세로 용지에서는 281mm 가로 폭 전제의 2단 grid가 양옆이 잘린다 — 모바일 인쇄에서는
`body.hlf-print-map-sizing .hlf-detail-hero`를 1단으로 접어 사진 위·지도 아래로 쌓는다. 지도 폭도
2단의 절반이 아니라 비교 지도와 같은 전체 폭(178mm)을 쓴다.

### 3. 매물 상세를 세로 용지 한 장에 2건씩 담는다
데스크톱은 매물 1건마다 페이지를 넘기지만(가로 용지 194mm 높이 기준으로 꽉 채움), 세로 용지는
높이가 281mm라 압축한 매물 상세 1건(~130mm)이 넉넉히 2건 들어간다. 인쇄 확정 시 JS가 화면에 보이는
매물 상세만 골라(`itemSections`) 짝의 두 번째 항목(0-based 홀수 인덱스)에 `hlf-print-pair-break`
클래스를 붙이고, print.css는 이 클래스가 있을 때만 페이지를 넘긴다(기본은 이어 붙임). 홀수 개가
남으면 마지막 1건은 그냥 혼자 마지막 페이지를 쓴다.

2건을 한 페이지에 넣기 위해 헤더(색띠+주소줄)/패널 여백/글자 크기/사진·지도 높이를 데스크톱
인쇄보다 한 단계 더 압축했다 — 개별 지도는 21mm, 대표사진은 최대 18mm로 낮췄다. Playwright 실측
(실제 템플릿과 동일한 헤더+히어로+Leasing Info+Property Details 10항목+푸터 구성, 지번주소
말줄임/특징 문구 등 긴 텍스트로 스트레스 테스트 포함): 2건이 정확히 A4 세로 1페이지(209.9x297.0mm)
안에 들어가고(약 3.4mm 여유), 3건은 2페이지(2건+1건)로, 데스크톱(가로, 2단 히어로, 1건/페이지)은
지금까지와 완전히 동일하게 유지됨을 확인했다.

### 4. NOC 차트가 인쇄에서 항상 줄바꿈 방식으로 나오도록 확정
모바일 화면 전용 규칙(`.hlf-noc-chart.is-dense`, 막대 5개 이상일 때 한 줄로 욱여넣는 nowrap 규칙)이
인쇄 렌더링에도 함께 적용되는지는 브라우저마다 확실하지 않다 — 이 규칙의 클래스 명시도가 인쇄
전용 규칙보다 높아, 만약 실제로 함께 적용된다면 인쇄에서도 의도치 않게 nowrap이 이길 수 있었다.
`!important`로 인쇄에서는 항상 줄바꿈+가운데 정렬로 확정했다(리스트 페이지 모바일 인쇄에서 지도가
"제대로 안 나온다"는 보고와 겹칠 수 있는 원인 중 하나로 판단해 함께 정리했다 — NOC 차트 자체가
아니라 이 media 조건 판정의 불확실성이 다른 요소에도 영향을 줄 수 있다고 봤다).

### 알아둘 점
- 이 라운드는 실제 모바일 기기의 인쇄 파이프라인을 직접 재현할 방법이 없어 Playwright(무헤드
  Chromium, `@page` 직접 적용 + PDF 페이지 수 실측)로 검증했다 — 실제 안드로이드/iOS 기기의 결과와
  다를 수 있다.
- 매물 상세를 목록 페이지의 선택 패널을 거치지 않고 단독으로 여는 페이지(`/list/{flyer}/{item}/`)를
  모바일에서 인쇄하면, 짝짓기 예산에 맞춰 압축된 지도(21mm)가 그대로 적용된다 — 항목이 하나뿐이라
  페이지 예산에는 여유가 넉넉하지만, 지도 자체가 필요 이상으로 작아 보일 수 있다(후속 개선 여지).
